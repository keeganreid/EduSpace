...
export function AuthProvider({ children }) {
    ...
    function signup(email, password) {
        return methods.createUserWithEmailAndPassword(auth, email, password).then(cred =>{
            if (cred !== undefined){
                let userType;
            if (email.substring(email.indexOf('@') + 1) === "sun.ac.za"){       //checks if a student email
                userType = "student";
            }
            else{
            ...
            let data = {
                type: userType,
                points : 500 //adding 500 points to a new user
            };
            return setDoc(doc(users, cred.user.uid), data); //sets a document id using that which is generated by the createuser function
    ...
    function login(email, password) {
        return methods.signInWithEmailAndPassword(auth, email, password) //firebase function to sign in
}
...
async function updateUserProfile(type, file, usernameRef, companyNameRef, companyURLRef, fullnameRef, facultyRef, degreeRef, bioRef) {
    ...
    photoURL = await getDownloadURL(fileRef);
        ...
        //firebase function to update a user profile, whereby a username and displayname associated with a user's authenticated profile can be set/changed
   await methods.updateProfile(currentUser, {photoURL, displayName: usernameRef.current.value});
}
else{ //if a photo is not uploaded, give the user a default URL
...
else
{
    ...
    return updateDoc(doc(users, currentUser.uid), data);  //update the user's correlating document in firestore
    
  }


    function logout() {  //firebase logout function
        return methods.signOut(auth)
    }

    useEffect(() => {
        const unsubscribe = methods.onAuthStateChanged(auth, (user) => {  
            //this checks if the authenticated state is changed
            setCurrentUser(user)
            setLoading(false)
        })
